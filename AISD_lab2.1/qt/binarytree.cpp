...
bool BinaryTree::deleteNode(Node* nd)
{
    return deleteNode(nd, findParent(nd));
}
...
BinaryTree::Node* BinaryTree::getFreeNode(Node* nd) const//ищет свободный член с одним узлом, надо исправить
{
    if (nd == nullptr)
...
void BinaryTree::printTree(Node* nd)
{
    ...
    for (int level = 0; level < height; level++)
    {
        ...
        for (int i = 0, tabs = 6; i < currentLevel.size(); i++, tabs--) {
            if (currentLevel[i])
                /*qInfo()*/ std::cout << currentLevel[i]->key << "  ";
            else
                /*qInfo()*/ std::cout  << '-' << ' ';
        }
        std::cout << '\n';
